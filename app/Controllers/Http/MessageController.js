...
class MessageController {
  ...
  async index ({ request, response, view }) {
    return new Promise(async (resolve, reject) => {
      ...
      if(filter.filter){
        this.req.get(`/messages?query=${filter.filter.query}&sort_by=created_at&sort_order=desc`).then(async (data) => {
          resolve(data.data.messages)
        }).catch((error) => {
          reject(error)
        })
      } else {
        this.req.get('/messages?sort_by=created_at&sort_order=desc').then(async (data) => {
          resolve(data.data.messages)
        }).catch((error) => {
          reject(error)
        })
