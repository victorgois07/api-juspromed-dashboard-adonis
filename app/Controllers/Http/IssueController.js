...
class IssueController {
  ...
  async index ({ request, response, view }) {
    return new Promise(async (resolve, reject) => {
      ...
      if(filter.filter){
        this.req.get(`/issues?query=${filter.filter.query}&sort_by=created_at&sort_order=desc`).then(async (data) => {
          resolve(data.data.issues)
        }).catch((error) => {
          reject(error)
        })
      } else {
        this.req.get('/issues?sort_by=created_at&sort_order=desc').then(async (data) => {
          resolve(data.data.issues)
        }).catch((error) => {
          reject(error)
        })
